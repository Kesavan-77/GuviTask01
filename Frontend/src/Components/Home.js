...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchHome = async () => {
      try {
        const response = await axios.get(`https://guvitask01.onrender.com/Home/${userId}`);
        if (response.data) {
          setHomeData(response.data);
          setIsEditMode(
            !response.data.age && !response.data.gender && !response.data.dob && !response.data.mobile
          );
        } else {
          setIsEditMode(true);
        }
  ...
  const handleSave = async () => {
    try {
      await axios.put(`https://guvitask01.onrender.com/Home/${userId}`, homeData);
      setIsEditMode(false);
    } catch (error) {
      console.error(error.response.data.message);
    }
